...
|---|---|
| Date | 2026-05-30 |
| Project | `SFunctor` / Phase 1 environmental census |
...
| Compute environment | Andes CPU partition `batch`, allocation `AST207`, QoS `normal`, one node per Phase 1 job |
...
![Phase 1 workflow schematic](figures/phase1_status_update/workflow_overview_schematic.png)

*Figure 0.1: Schematic of the completed Phase 1 workflow. The reader should
...
![dBB map on one census plane](figures/phase1_status_update/dBB_L80_midplane_map.png)
...
![dBB trend with scale](figures/phase1_status_update/dBB_quantile_trend_by_Lsub.png)
...
![Domain tiling schematic](figures/phase1_status_update/domain_tiling_schematic.png)
...
![Primary-only workflow change](figures/phase1_status_update/primary_only_workflow_change_schematic.png)
...
![Primitive-to-cbin residuals](figures/phase1_status_update/validation_raw_moment_residuals.png)
...
![dBB distributions](figures/phase1_status_update/dBB_distribution_by_Lsub.png)
...
![Magnetic correlations](figures/phase1_status_update/dBB_magnetic_correlations_L640.png)
...
![Supported non-magnetic scale trends](figures/phase1_status_update/environmental_quantile_trends_by_Lsub.png)
...
The continuation is deliberately staged. Phase 2 should implement and validate
a selected-cube 3-D extractor, benchmark a few $L_{\rm sub}=640$ cold reads,
and compare extracted moments with the catalog. Phase 3 should validate
finite-domain pair samplers and run the smallest structure-function smoke
test. Only then should Phase 4 decide whether the bounded 21-region pilot is
...
`scripts/phase1/cbin_tools.py` provides the binary reader and numerical
utilities:
...
`scripts/phase1/run_phase1_catalog.py` orchestrates build and analysis:
...
The Phase 1 Slurm jobs reserve one Andes node and `32` CPUs. The current
...
`scripts/phase1/verify_phase1_outputs.py` is a structured second path. It
...
`scripts/phase1/update_compute_ledger.py` maintains persistent CSV and
...
`scripts/phase1/validate_reconstruction.py` converts limited primitive-data
...
![Flagged-row fraction](figures/phase1_status_update/catalog_flagged_fraction_by_Lsub.png)
...
`docs/PHASE1_RECONSTRUCTABILITY.md` for the moment-flag bit mapping.
...
The Phase 1-focused subset passed:
...
![Conditional magnetic bands](figures/phase1_status_update/conditional_dBB_magnetic_quintiles_L640.png)
...
![Supported non-magnetic dBB correlations](figures/phase1_status_update/dBB_nonmagnetic_correlations_L640.png)
...
![Supported-property correlation matrix](figures/phase1_status_update/spearman_correlation_matrix_L640.png)
...
The audit attempted the additional Phase 1 quantities but did not manufacture
...
`docs/PHASE1_RECONSTRUCTABILITY.md`.

### Nested-grid persistence

![Cross-scale dBB persistence](figures/phase1_status_update/cross_scale_dBB_persistence.png)
...
![Pilot selection](figures/phase1_status_update/pilot_selection_L640.png)
...
![Extraction scaling estimates](figures/phase1_status_update/pilot_extraction_scaling_estimates.png)
...
The catalogs and future-stage phase documents now use `1280`.
...
1. Implement or audit a selected-cube 3-D primitive extractor. The existing
   production extraction path is for 2-D slices, not this Phase 2 cube
   benchmark.
...
6. In Phase 3, implement nested-core and all-valid-pairs finite-domain
   samplers. Validate
...
7. In Phase 3, run the smallest scientifically useful structure-function
   smoke test.
8. Expand to the bounded Phase 4 21-region pilot only after the benchmark and
   smoke test pass.
9. Require explicit human approval before the Phase 5 cross-scale campaign.
10. Treat restoration of SGS-derived or dynamo-derivative quantities as a
...
Phase 1 addressed a practical prerequisite for a later structure-function
...
structure function has been calculated. The next safe action is the bounded
Phase 2 extractor and cold-read benchmark for a minimal $L_{\rm sub}=640$
subset. Phase 3 finite-domain sampler work follows only after that benchmark
passes. Do not restore broken SGS products and do not mutate the trusted final
run while preparing that benchmark.
...
Implementation commit:
  ...
  Add Phase 1 catalog census workflow
```

The implementation baseline was pushed and synchronized with
`origin/cleanup/cpu-production`. This Phase 1 status report, its figures, its
...
```text
docs/PHASE1_DATA_LAYOUT.md
docs/PHASE1_EXECUTION_REPORT.md
docs/PHASE1_RECONSTRUCTABILITY.md
docs/PHASE1_UNRESOLVED.md
job_scripts/phase1/README.md
job_scripts/phase1/run_phase1_andes.sh
job_scripts/phase1/run_phase1_verify_andes.sh
scripts/phase1/__init__.py
scripts/phase1/cbin_tools.py
scripts/phase1/run_phase1_catalog.py
scripts/phase1/update_compute_ledger.py
scripts/phase1/validate_reconstruction.py
scripts/phase1/verify_phase1_outputs.py
tests/test_compute_ledger.py
tests/test_phase1_analysis.py
tests/test_phase1_cbin_tools.py
```
...
The phase edits corrected every stale `1240` reference to `1280`. The two
production Slurm templates were updated to remove email notifications.

Subsequent continuation planning split the formerly oversized Phase 2
campaign into:
...
with the Phase 1 scripts. Use the activated Python `3.11` environment.
...
```bash
python scripts/phase1/generate_phase1_status_update_figures.py
```
...
python scripts/phase1/generate_phase1_status_update_figures.py \
  ...
  --output-dir figures/phase1_status_update
```

The helper writes `16` PNG files and
`figures/phase1_status_update/figure_manifest.json`. It does not modify the trusted
run directory.
...
```bash
RUN_DIR=/lustre/orion/ast207/proj-shared/dfielding/Production_plm/phase1_catalog/t6_repro_UNIQUE
```
...
python scripts/phase1/verify_phase1_outputs.py \
  --run-dir "$TRUSTED_RUN" \
  --output-dir "$(mktemp -d)"
```
...
```bash
mkdir -p "$RUN_DIR"
python scripts/phase1/run_phase1_catalog.py probe \
  --output "$RUN_DIR/data_layout_probe.json"
python scripts/phase1/validate_reconstruction.py \
  --output-dir "$RUN_DIR/validation"
python scripts/phase1/run_phase1_catalog.py build \
  --run-dir "$RUN_DIR"
python scripts/phase1/verify_phase1_outputs.py \
  --run-dir "$RUN_DIR"
python scripts/phase1/run_phase1_catalog.py analyze \
  --run-dir "$RUN_DIR"
```

### Compute ledger

The retained ledger lives at the legacy physical path below. The repository
adopted phase terminology after the completed run; the validated Lustre tree
was intentionally not moved.
...
production extractor is for 2-D slices. Implement and validate the Phase 2
...
python scripts/phase1/update_compute_ledger.py \
  ...
  --no-refresh \
  --register-planned \
  --job-name phase2_extract_smoke_L640 \
...
No random seed is used by the Phase 1 census or pilot selector. The catalogs
...
| Path | Description |
|---|---|
| `PHASE1_STATUS_UPDATE.md` | standalone multi-tier status report |
| `scripts/phase1/generate_phase1_status_update_figures.py` | reproducible report-figure generator |
| `figures/phase1_status_update/figure_manifest.json` | report metrics plus SHA256 hashes for the generator, quantitative inputs, and all new figures |
| `figures/phase1_status_update/*.png` | `16` new report figures |
...
13. This report, its figure generator, and `figures/phase1_status_update/`
...
1. Read `PHASE1_STATUS_UPDATE.md`, `docs/PHASE1_EXECUTION_REPORT.md`, and
   `docs/PHASE1_RECONSTRUCTABILITY.md` first. Then define:
...
2. Treat only
   `/lustre/orion/ast207/proj-shared/dfielding/Production_plm/prompt1_catalog/t6_final_primary_20260530`
   as the trusted Phase 1 run. Treat it as read-only.
...
6. Implement and validate the selected-cube 3-D extractor, then begin with
   ...
   Tier 2.
7. Follow `phase3.md`: implement and validate nested-core and all-valid-pairs
   finite-domain samplers against a slow 3-D oracle and synthetic edge cases
   before calculating production structure functions.
8. Require the full repository test suite and the smallest Phase 3
   structure-function smoke test to pass before scaling to the bounded
   `phase4.md` 21-region pilot.
9. Ask for human input before beginning the `phase5.md` expansion campaign,
   adding snapshots, restoring excluded channels, or launching expensive
   $L_{\rm sub}=1280$ extraction work.
...
|---|---|---|---|
| 0.1 | `figures/phase1_status_update/workflow_overview_schematic.png` | schematic | explain the validated primary-only workflow |
| 0.2 | `figures/phase1_status_update/dBB_L80_midplane_map.png` | quantitative | show spatial structure in the actual census |
| 0.3 | `figures/phase1_status_update/dBB_quantile_trend_by_Lsub.png` | quantitative | summarize the main scale trend |
| 1.1 | `figures/phase1_status_update/domain_tiling_schematic.png` | schematic | explain exact tiling and pilot-scale choice |
| 1.2 | `figures/phase1_status_update/primary_only_workflow_change_schematic.png` | schematic | explain the SGS exclusion and rebuild |
| 1.3 | `figures/phase1_status_update/validation_raw_moment_residuals.png` | quantitative | show direct validation residuals |
| 1.4 | `figures/phase1_status_update/dBB_distribution_by_Lsub.png` | quantitative | show scale-dependent distributions |
| 1.5 | `figures/phase1_status_update/dBB_magnetic_correlations_L640.png` | quantitative | show weak-mean-field and elevated-fluctuation ratio associations |
| 1.6 | `figures/phase1_status_update/environmental_quantile_trends_by_Lsub.png` | quantitative | show supported density, conserved-momentum, and coarse mass-weighted velocity trends |
| 2.1 | `figures/phase1_status_update/catalog_flagged_fraction_by_Lsub.png` | quantitative | show precision-flag prevalence |
| 2.2 | `figures/phase1_status_update/conditional_dBB_magnetic_quintiles_L640.png` | quantitative | show conditional magnetic trends |
| 2.3 | `figures/phase1_status_update/dBB_nonmagnetic_correlations_L640.png` | quantitative | show supported density and conserved-momentum associations with $\mathrm{dBB}$ |
| 2.4 | `figures/phase1_status_update/spearman_correlation_matrix_L640.png` | quantitative | show the broader supported-property covariance structure |
| 2.5 | `figures/phase1_status_update/cross_scale_dBB_persistence.png` | quantitative | quantify cross-scale persistence |
| 2.6 | `figures/phase1_status_update/pilot_selection_L640.png` | quantitative | show representative, matched, and outlier targets |
| 2.7 | `figures/phase1_status_update/pilot_extraction_scaling_estimates.png` | estimate | plan later extraction benchmarking |
...
| Full adversarial review | completed | Renamed the status to census complete with pilot proposed, required extractor and sampler validation gates before the 21-region pilot, recorded superseded-run accounting, expanded shifted-grid and held-out validation risks, aligned the execution report, and bound generator plus input hashes into the figure manifest. |

# Appendix C: Current concise status

The Phase 1 primary-only environmental census is complete, validated, and
...
next action is the bounded Phase 2 selected-cube 3-D extractor and a small
ledger-registered $L_{\rm sub}=640$ cold-read extraction benchmark. Phase 3
finite-domain sampler validation follows only after that gate passes.

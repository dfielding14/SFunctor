...
def load_first_file_metadata(hist_file):
    ...
    if 'delta_bin_edges' in data:
        result['delta_bin_edges'] = data['delta_bin_edges']

    return result


def reconstruct_bin_edges(metadata):
    """Reconstruct delta bin edges from metadata if needed."""
    delta_bin_edges = None
...
def combine_node_histograms_parallel(histogram_files, args):
    ...
    if delta_bin_edges is None:
        delta_bin_edges = reconstruct_bin_edges(metadata)
    ...
    if len(histogram_files) > 1:
        ...
        for result in tqdm(results, desc="Summing", disable=not HAS_TQDM):
            ...
            np.add(hist_total, result['hist'], out=hist_total)

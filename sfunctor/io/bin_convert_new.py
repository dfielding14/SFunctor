...
def read_binary_metadata(filename: str) -> Dict[str, Any]:
    """Read rank-local block ownership and geometry without loading cell data."""

    return _read_binary_metadata_impl(filename)
...
def _read_binary_metadata_impl(filename: str) -> Dict[str, Any]:
    """Scan one Athena binary dump while seeking over each cell-data payload."""
    ...
    with open(filename, "rb") as fp:
        ...
        bounds = tuple(
            float(_get_from_header(header_lines, "<mesh>", name))
            for name in ("x1min", "x1max", "x2min", "x2max", "x3min", "x3max")
        )
        ...
        while fp.tell() < filesize:
            ...
            if len(raw_index) != 24:
                raise IOError("Unexpected EOF while reading mesh-block indices")
            index = np.frombuffer(raw_index, dtype=np.int32).astype(np.int64) - nghost
            raw_logical = fp.read(16)
            raw_geometry = fp.read(6 * locsizebytes)
            if len(raw_logical) != 16 or len(raw_geometry) != 6 * locsizebytes:
                raise IOError("Unexpected EOF while reading mesh-block metadata")
            logical = np.frombuffer(raw_logical, dtype=np.int32)
            geometry = np.frombuffer(raw_geometry, dtype=dtype_loc)
            nx1_out = int(index[1] - index[0] + 1)
            nx2_out = int(index[3] - index[2] + 1)
            nx3_out = int(index[5] - index[4] + 1)
            if min(nx1_out, nx2_out, nx3_out) <= 0:
                raise ValueError("Mesh-block indices imply a non-positive block extent")
            bytes_needed = nx1_out * nx2_out * nx3_out * nvars * varsizebytes
            fp.seek(bytes_needed, os.SEEK_CUR)
            if fp.tell() > filesize:
                raise IOError("Unexpected EOF while seeking over mesh-block data")
    ...
    return {
        ...
        "nx2_mb": nx2,
        "nx3_mb": nx3,
        "mb_index": np.asarray(mb_index),
        "mb_logical": np.asarray(mb_logical),
        "mb_geometry": np.asarray(mb_geometry),
    }

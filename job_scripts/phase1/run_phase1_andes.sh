#!/bin/bash
#SBATCH -A AST207
#SBATCH -J phase1_cbin
#SBATCH -o logs/phase1_%x_%j.out
#SBATCH -e logs/phase1_%x_%j.err
#SBATCH -t 04:00:00
#SBATCH -p batch
#SBATCH -N 1
...
LOCK_DIR="${RUN_DIR}/.phase1_action_lock"
if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "ERROR: another Phase 1 action holds ${LOCK_DIR}" >&2
  exit 3
fi
trap 'rmdir "${LOCK_DIR}"' EXIT
log "Phase 1 cbin workflow"
log "Action: ${ACTION}"
...
case "${ACTION}" in
  validate)
    command=(
      python "${SFUNCTOR_DIR}/scripts/phase1/validate_reconstruction.py"
      --output-dir "${RUN_DIR}/validation"
    )
    ;;
  build)
    command=(
      python "${SFUNCTOR_DIR}/scripts/phase1/run_phase1_catalog.py"
      build
      --run-dir "${RUN_DIR}"
    )
    ;;

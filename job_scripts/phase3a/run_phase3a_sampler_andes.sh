...
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase3a_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 3a action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

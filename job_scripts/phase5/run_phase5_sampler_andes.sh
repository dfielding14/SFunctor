#!/usr/bin/env bash
#SBATCH -A AST207
#SBATCH -J phase5_sampler
#SBATCH -o logs/phase5_sampler_%x_%j.out
#SBATCH -e logs/phase5_sampler_%x_%j.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 1
...
: "${CAMPAIGN_CONFIG:?CAMPAIGN_CONFIG must be the approved bounded Phase 5 campaign JSON}"
: "${SCALE:?SCALE must be 80, 160, or 320}"
: "${SELECTION_SET:?SELECTION_SET must name one approved Phase 5 selection set}"
: "${MATRIX:?MATRIX must be baseline, orders, 3point, or 5point}"
: "${PHASE2_ROOT:?PHASE2_ROOT must be the explicit retained Phase 5 extraction directory}"
: "${OUTPUT_ROOT:?OUTPUT_ROOT must be an explicit unique Phase 5 sampler directory}"
...
CLI="${SFUNCTOR_DIR}/scripts/phase5/run_phase5_sampler.py"
PYTHON_BIN="${SFUNCTOR_DIR}/venv_sfunctor/bin/python"
PHASE5_WORKERS="${PHASE5_WORKERS:-1}"

if [[ -e "${RUN_DIR}" ]]; then
  echo "ERROR: Phase 5 sampler allocation directory already exists: ${RUN_DIR}" >&2
  exit 3
fi

cd "${SFUNCTOR_DIR}"
mkdir -p logs "$(dirname -- "${OUTPUT_ROOT}")"
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase5_sampler_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 5 sampler action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 5 sampler action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT
if ! mkdir "${RUN_DIR}"; then
  echo "ERROR: Phase 5 sampler allocation directory appeared concurrently: ${RUN_DIR}" >&2
  exit 3
fi
RUN_DIR_CREATED=1
mkdir "${RUN_DIR}/logs"
...
case "${ACTION}" in
  work)
    /usr/bin/time -v srun --ntasks="${SLURM_JOB_NUM_NODES:-1}" --ntasks-per-node=1 \
      ...
      "$PYTHON_BIN" "${CLI}" work "${COMMON[@]}" > "${RUN_DIR}/logs/work.log" 2>&1
    ;;

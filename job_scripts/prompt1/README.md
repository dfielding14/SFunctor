...
Use `scripts/prompt1/update_compute_ledger.py` to keep the prompt1 workflow's
...
python scripts/prompt1/update_compute_ledger.py \
  ...
  --output-dir Results/prompt1/sf_smoke_640
```
...
```bash
python scripts/prompt1/update_compute_ledger.py --results-dir Results/prompt1
```
...
- `run_prompt1_andes.sh`: direct reconstruction validation, streamed catalog
  build, and analysis actions selected with `ACTION`.
- `run_prompt1_verify_andes.sh`: read-only independent verification of

...
# Check if run_node_analysis.py can be imported
echo "Checking run_node_analysis.py..."
python -c "import sys; sys.path.insert(0, '/autofs/nccs-svm1_home2/dfielding/SFunctor'); import run_node_analysis; print('run_node_analysis.py imports successfully')"
...
if [ -f "$SLICE_PATH" ]; then
    ...
    python /autofs/nccs-svm1_home2/dfielding/SFunctor/generate_displacements.py \
        --n_disp_total 100 \
        --n_ell_bins 10 \
        --Nres $GRID_RES \
        --stencil_width 2 \
    ...
    python /autofs/nccs-svm1_home2/dfielding/SFunctor/run_node_analysis.py \
        --slice "$SLICE_PATH" \

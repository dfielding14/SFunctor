...
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase4_batch_b_tail_diagnostic_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 4 Batch B tail-diagnostic action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 4 Batch B tail-diagnostic action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT
...
case "${ACTION}" in
  work)
    /usr/bin/time -v srun --ntasks="${SLURM_JOB_NUM_NODES:-1}" --ntasks-per-node=1 \
      --cpus-per-task="${SLURM_CPUS_PER_TASK:-1}" --cpu-bind=cores \

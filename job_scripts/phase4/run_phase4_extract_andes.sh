#!/usr/bin/env bash
#SBATCH -A AST207
#SBATCH -J phase4_extract
#SBATCH -o logs/phase4_extract_%x_%j.out
#SBATCH -e logs/phase4_extract_%x_%j.err
#SBATCH -t 02:30:00
#SBATCH -p batch
#SBATCH -N 1
...
: "${OUTPUT_ROOT:?OUTPUT_ROOT must be an explicit unique Phase 4 extraction directory}"
: "${RUN_DIR:?RUN_DIR must be an explicit unique allocation directory}"
: "${ACTION:?ACTION must be plan, extract, verify, or inspect}"
CLI="${SFUNCTOR_DIR}/scripts/phase4/run_phase4_extraction.py"
PYTHON_BIN="${SFUNCTOR_DIR}/venv_sfunctor/bin/python"
PHASE4_CLEAN_PARTIAL="${PHASE4_CLEAN_PARTIAL:-0}"
PHASE4_CLEAN_INCOMPLETE="${PHASE4_CLEAN_INCOMPLETE:-0}"
PHASE4_CLEAN_STALE_LOCK="${PHASE4_CLEAN_STALE_LOCK:-0}"

cd "${SFUNCTOR_DIR}"
mkdir -p logs "${RUN_DIR}/logs" "$(dirname -- "${OUTPUT_ROOT}")"
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase4_extract_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 4 extraction action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 4 extraction action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT
...
EXTRACT_FLAGS=()
[[ "${PHASE4_CLEAN_PARTIAL}" == "1" ]] && EXTRACT_FLAGS+=(--clean-partial)
[[ "${PHASE4_CLEAN_INCOMPLETE}" == "1" ]] && EXTRACT_FLAGS+=(--clean-incomplete)
[[ "${PHASE4_CLEAN_STALE_LOCK}" == "1" ]] && EXTRACT_FLAGS+=(--clean-stale-lock)
...
case "${ACTION}" in
  plan)
    ...
    ;;
  extract|verify|inspect)
    run_python "${ACTION}" "${COMMON[@]}" --all-pilot "${EXTRACT_FLAGS[@]}" > "${RUN_DIR}/logs/${ACTION}.log" 2>&1
    ;;
  *)

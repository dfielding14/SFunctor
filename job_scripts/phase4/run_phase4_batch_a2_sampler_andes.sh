#!/usr/bin/env bash
#SBATCH -A AST207
#SBATCH -J phase4_batch_a2
#SBATCH -o logs/phase4_batch_a2_%x_%j.out
#SBATCH -e logs/phase4_batch_a2_%x_%j.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 1
...
CLI="${SFUNCTOR_DIR}/scripts/phase4/run_phase4_batch_a2_sampler.py"
...
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase4_batch_a2_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 4 Batch A2 action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 4 Batch A2 action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT

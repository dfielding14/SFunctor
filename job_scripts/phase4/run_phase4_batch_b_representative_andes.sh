#!/usr/bin/env bash
#SBATCH -A AST207
#SBATCH -J phase4_batch_b_representative
#SBATCH -o logs/phase4_batch_b_representative_%x_%j.out
#SBATCH -e logs/phase4_batch_b_representative_%x_%j.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 1
...
: "${OUTPUT_ROOT:?OUTPUT_ROOT must be an explicit unique Phase 4 Batch B representative directory}"
...
CLI="${SFUNCTOR_DIR}/scripts/phase4/run_phase4_batch_b_representative_sampler.py"
PYTHON_BIN="${SFUNCTOR_DIR}/venv_sfunctor/bin/python"
PHASE4_BATCH_B_WORKERS="${PHASE4_BATCH_B_WORKERS:-1}"

cd "${SFUNCTOR_DIR}"
mkdir -p logs "${RUN_DIR}/logs" "$(dirname -- "${OUTPUT_ROOT}")"
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase4_batch_b_representative_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 4 Batch B representative action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 4 Batch B representative action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT
...
case "${ACTION}" in
  work)
    /usr/bin/time -v srun --ntasks="${SLURM_JOB_NUM_NODES:-1}" --ntasks-per-node=1 \
      ...
      --output-root "${OUTPUT_ROOT}" --workers "${PHASE4_BATCH_B_WORKERS}" \

#!/usr/bin/env bash
#SBATCH -A AST207
#SBATCH -J phase4_batch_a2_3point_extension
#SBATCH -o logs/phase4_batch_a2_3point_extension_%x_%j.out
#SBATCH -e logs/phase4_batch_a2_3point_extension_%x_%j.err
#SBATCH -t 02:00:00
#SBATCH -p batch
#SBATCH -N 1
...
: "${OUTPUT_ROOT:?OUTPUT_ROOT must be an explicit unique Phase 4 Batch A2 directory}"
...
CLI="${SFUNCTOR_DIR}/scripts/phase4/run_phase4_batch_a2_3point_extension.py"
PYTHON_BIN="${SFUNCTOR_DIR}/venv_sfunctor/bin/python"
PHASE4_A2_WORKERS="${PHASE4_A2_WORKERS:-1}"

cd "${SFUNCTOR_DIR}"
mkdir -p logs "${RUN_DIR}/logs" "$(dirname -- "${OUTPUT_ROOT}")"
LOCK_DIR="$(dirname -- "${OUTPUT_ROOT}")/.$(basename -- "${OUTPUT_ROOT}").phase4_batch_a2_3point_extension_action_lock"
...
recover_stale_lock() {
  ...
  echo "Recovered stale Phase 4 Batch A2 3-point extension action lock for inactive Slurm job ${owner_job_id}: ${LOCK_DIR}" >&2
}

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  if ! recover_stale_lock || ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "ERROR: another Phase 4 Batch A2 3-point extension action holds ${LOCK_DIR}" >&2
    exit 3
  fi
fi
trap finalize EXIT
...
case "${ACTION}" in
  work)
    /usr/bin/time -v srun --ntasks="${SLURM_JOB_NUM_NODES:-1}" --ntasks-per-node=1 \
      ...
      "$PYTHON_BIN" "${CLI}" work --phase2-root "${PHASE2_ROOT}" --batch-a-root "${BATCH_A_ROOT}" --representative-a2-root "${REPRESENTATIVE_A2_ROOT}" \
      --output-root "${OUTPUT_ROOT}" --workers "${PHASE4_A2_WORKERS}" \
      > "${RUN_DIR}/logs/work.log" 2>&1
    ;;
  plan|reduce|verify|summarize)
    run_python "${ACTION}" --phase2-root "${PHASE2_ROOT}" --batch-a-root "${BATCH_A_ROOT}" --representative-a2-root "${REPRESENTATIVE_A2_ROOT}" \
      --output-root "${OUTPUT_ROOT}" --workers "${PHASE4_A2_WORKERS}" \

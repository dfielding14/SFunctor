...
# Multi-node slice extraction: each task handles a subset of slices independently.
# Usage:
#   sbatch -N <nodes> --job-name="slice_${SIM_NAME}" \
#     --export=ALL,SIM_NAME=...,FILE_NUMBER=-1 job_scripts/production/run_slice_extraction_frontier_multi.sh
...
if [[ -z "$ELONG_Z_FACTOR" ]]; then
    # Only elongated beta=1 runs; avoid matching beta10/beta100
...
POS_LIST_STR="-0.375 -0.25 -0.125 0.0 0.125 0.25 0.375 0.5"

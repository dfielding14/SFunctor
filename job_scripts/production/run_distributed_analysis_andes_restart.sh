...
# - Reuses displacements.npz if present
# - If a slice has a full set of node histograms, it only recombines them
# - If histograms are incomplete, it re-runs that slice from scratch
# Usage:
#   sbatch --export=ALL,SIM_NAME=...,STENCIL_WIDTH=...,N_DISP_TOTAL=...,N_RANDOM_SUBSAMPLES=... \
...
RUN_NAME_DEFAULT="ndisp${N_DISP_TOTAL}_nrand${N_RANDOM_SUBSAMPLES}_nell${N_ELL_BINS}_sw${STENCIL_WIDTH}_job${SLURM_JOB_ID:-manual}"
RUN_NAME="${RUN_NAME_OVERRIDE:-$RUN_NAME_DEFAULT}"

if [[ -n "${RESUME_DIR:-}" ]]; then
    WORK_DIR="${RESUME_DIR}"
    log "RESUME mode: using existing work directory ${WORK_DIR}"
else
    WORK_DIR="${RESULTS_BASE}/results_${SIM_NAME}/${RUN_NAME}"
    log "NEW RUN mode: writing to ${WORK_DIR}"
fi

SLICE_LIST="${SLICES_BASE}/slice_list_${SIM_NAME}.txt"

log "============================================================"
log "Restartable distributed analysis (Andes)"
...
if [ -f "$WORK_DIR/sf_results_all_slices.npz" ]; then
    log "Final combined file already present ($WORK_DIR/sf_results_all_slices.npz). Exiting."
    exit 0
fi
...
# Helper to read expected node count from the first histogram file (if any)
expected_nodes_from_hist() {
    local hist_file="$1"
    "$PYTHON_BIN" - <<'PY' "$hist_file"
import sys, numpy as np
path = sys.argv[1]
try:
    data = np.load(path, allow_pickle=True)
    info = data.get('node_info', None)
    if info is None:
        print(0)
    else:
        info = dict(info.item())
        print(info.get('total_nodes', 0))
except Exception:
    print(0)
PY
}
...
for ((SLICE_IDX=0; SLICE_IDX<${#ALL_SLICES[@]}; SLICE_IDX++)); do
    ...
    SLICE_RESULT="$WORK_DIR/sf_results_${SLICE_NAME}.npz"

    log "-----"
    log "Slice ${SLICE_NAME} ($((SLICE_IDX + 1))/${TOTAL_SLICES})"

    if [ -f "$SLICE_RESULT" ]; then
        log "Found existing slice result ($SLICE_RESULT); skipping."
        continue
    fi
    ...
    if [ -d "$SLICE_OUTPUT_DIR" ]; then
        mapfile -t HIST_FILES < <(find "$SLICE_OUTPUT_DIR" -maxdepth 1 -name "histogram_*.npz" | sort)
        HIST_COUNT=${#HIST_FILES[@]}
        if (( HIST_COUNT > 0 )); then
            EXPECTED_HIST_NODES=$(expected_nodes_from_hist "${HIST_FILES[0]}")
            if (( HIST_COUNT == EXPECTED_HIST_NODES )) && (( EXPECTED_HIST_NODES > 0 )); then
                log "Found complete histogram set (${HIST_COUNT}/${EXPECTED_HIST_NODES}); will recombine without rerunning nodes."
                RUN_NODE_TASKS=false
            else
                log "Incomplete histogram set (${HIST_COUNT}/${EXPECTED_HIST_NODES}); removing and recomputing."
